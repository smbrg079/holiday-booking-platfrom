    ...
    return (
        <div className="sticky top-28 p-8 border border-slate-100 rounded-[2.5rem] bg-white shadow-2xl shadow-indigo-500/5">
            <div className="flex items-end justify-between mb-8">
                <div>
                    <p className="text-sm text-slate-400 font-bold uppercase tracking-wider mb-1">Price per person</p>
                    <p className="text-4xl font-black text-slate-900">{formatPrice(price)}</p>
                </div>
                <div className="text-right">
                    <p className="text-emerald-500 font-bold text-sm">Best Price Guarantee</p>
                </div>
            </div>

            <form onSubmit={handleBooking} className="space-y-6">
                <div>
                    <label className="block text-sm font-bold text-slate-900 mb-2">Select Date & Time</label>
                        ...
                        >
                            {slots.map((slot) => (
                                <option key={slot.id} value={slot.id}>
                                    {new Date(slot.startTime).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                </option>
                            ))}
                            {slots.length === 0 && <option disabled>No available slots</option>}
                        </select>
                    </div>
                </div>

                <div>
                    <label className="block text-sm font-bold text-slate-900 mb-2">Number of Participants</label>
                    <div className="relative">
                ...
                <div className="pt-4 border-t border-slate-100">
                    <div className="flex items-center justify-between mb-2 text-sm text-slate-500">
                        <span>{formatPrice(price)} x {participants} persons</span>
                    ...
                    >
                        {isSubmitting ? (
                            ...
                            <>
                                <span>Book Activity Now</span>
                                <ShieldCheck size={20} />
                            </>
                        )}
                    </button>
                    <p className="text-center text-xs text-slate-400 mt-4 font-medium">No risk: Free cancellation up to 24h before</p>
                </div>
            </form>
        </div>
    )
